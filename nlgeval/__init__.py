...
def compute_metrics(hypothesis, references, no_overlap=False, no_skipthoughts=True, no_glove=True, logger=None):
		...
		for scorer, method in scorers:
			...
			if isinstance(method, list):
				for sc, scs, m in zip(score, scores, method):
					show_msg("%s: %0.6f" % (m, sc), logger)
					ret_scores[m] = sc
			else:
				show_msg("%s: %0.6f" % (method, score), logger)
	...
	if not no_skipthoughts:
		...
		show_msg("SkipThoughtsCosineSimilairty: %0.6f" % (cosine_similarity), logger)
