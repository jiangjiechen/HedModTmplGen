...
from monmt.utils.loss import LossComputeBase
...
class CopyGenerator(nn.Module):
	...
	* :math:`p_{copy}` the probility of copying a particular word.
	...
	def forward(self, hidden, attn, src_map):
		...
		extended by the dynamic dictionary implied by complying
		source words.
...
class CopyGeneratorLossCompute(LossComputeBase):
	...
	def _make_shard_state(self, batch, output, range_, attns):
		...
		return {
			...
			# TODO: copygenerator uses tgt by default
			"copy_attn": attns.get("copy"),
			"align": batch.alignment[range_[0] + 1: range_[1]]
		}

	def _compute_loss(self, batch, output, target, copy_attn, align, show=False):
		"""
		...
		if show:
			preds = scores_data.max(1)[1]
			return loss, (stats, preds)
		else:
			return loss, stats
