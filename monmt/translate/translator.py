...
from monmt.utils.misc import tile

import monmt.model_builder
import monmt.translate.beam
import monmt.inputters as inputters
import monmt.decoders.ensemble
...
def build_translator(opt, report_score=True, logger=None, out_file=None):
	if out_file is None:
		output = opt.output if len(opt.models) == 2 else opt.src2
		out_file = codecs.open(output, 'w', 'utf-8')

	fields, models, model_opt = monmt.model_builder.load_test_model(opt)
	...
	scorer = monmt.translate.GNMTGlobalScorer(opt)
...
class Translator(object):
	...
	Args:
	   model (:obj:`monmt.modules.NMTModel`):
	...
	def __init__(
		...
		self.tgt_file = opt.tgt
		self.pred_file = opt.output
		self.fast = False  # TODO: newest opennmt version not supported yet
		...
		self.joint = opt.joint if hasattr(opt, 'joint') else False
		self.pass1 = True if self.model2 is None else False
	...
	def translate(
		...
		builder = monmt.translate.TranslationBuilder(
		...
		for batch in data_iter:
			batch_data = self.translate_batch(
				batch, data, attn_debug, fast=self.fast, pass1=self.pass1
			)
		...
		if self.report_score:
			...
			if tgt is not None:
				...
				if self.report_rouge or self.report_bleu:
					...
					compute_metrics(self.pred_file, [self.tgt_file], logger=self.logger)
	...
	def translate_batch(self, batch, data, attn_debug, fast=False, pass1=False):
		"""
		...
		assert not fast  # TODO: not support this yet
		with torch.no_grad():
			if fast:
				...
				return self._translate_batch(batch, data, pass1)

	def _run_encoder(self, batch, pass1):
		src_name = 'src' if pass1 else 'src2'
		src = inputters.make_features(batch, src_name)
		if pass1:
			_, src_lengths = batch.src
			enc_states, memory_bank, src_lengths = self.model.encoder(src, src_lengths)
		else:
			_, src_lengths = batch.src2
			enc_states, memory_bank, src_lengths = self.model2.encoder(src)
	...
	def _translate_batch(self, batch, data, pass1=False):
		# (0) Prep each of the components of the search.
		# And helper method for reducing verbosity.
		beam_size = self.beam_size
		batch_size = batch.batch_size

		tgt_name = 'tmpl' if pass1 else 'tgt'
		vocab = self.fields[tgt_name].vocab

		# Define a set of tokens to exclude from ngram-blocking
		exclusion_tokens = {vocab.stoi[t] for t in self.ignore_when_blocking}

		pad = vocab.stoi[self.fields[tgt_name].pad_token]
		eos = vocab.stoi[self.fields[tgt_name].eos_token]
		bos = vocab.stoi[self.fields[tgt_name].init_token]
		beam = [monmt.translate.Beam(beam_size, n_best=self.n_best,
		                             cuda=self.cuda,
		...
		src, enc_states, memory_bank, src_lengths = self._run_encoder(batch, True)

		model = self.model
		if not pass1:
			memory_bank1 = memory_bank
			memory_lengths1 = src_lengths
			src, enc_states, memory_bank, src_lengths = self._run_encoder(batch, False)
			model = self.model2
		else:
			memory_lengths1 = None
			memory_bank1 = None
		...
		# if 'tgt' in batch.__dict__ and not pass1:
		# 	results["gold_score"] = self._score_target(
		# 		batch, memory_bank, src_lengths, data, batch.src_map
		# 		if self.copy_attn else None, pass1=False)
		# 	self.model.decoder.init_state(src, memory_bank, enc_states)
		# else:
		# 	results["gold_score"] = [0] * batch_size
		results["gold_score"] = [0] * batch_size
		...
		src_map = (tile(batch.src_map, beam_size, dim=1)
		           if self.copy_attn and not pass1 else None)
		...
		if memory_lengths1 is not None and memory_bank1 is not None:
			memory_lengths1 = tile(memory_lengths1, beam_size)
			if isinstance(memory_bank, tuple):
		...
		for i in range(self.max_length):
			...
			out, beam_attn = self._decode_and_generate(
				...
				src_map=src_map, step=i, pass1=pass1,
				memory_bank1=memory_bank1, memory_lengths1=memory_lengths1,
			)
	...
	def _decode_and_generate(
		...
		assert (pass1 and memory_bank1 is None) or \
		       (not pass1 and memory_bank1 is not None)
		unk_idx = self.fields["tgt"].vocab.stoi[self.fields["tgt"].unk_token]
		if self.copy_attn and not pass1:
		...
		if pass1:
			...
			dec_out, dec_attn = self.model2.decoder(
				decoder_in, memory_bank1=memory_bank1, memory_bank2=memory_bank,
				memory_lengths1=memory_lengths1, memory_lengths2=memory_lengths,
				step=step
			)

		model = self.model if pass1 else self.model2
		# Generator forward.
		if not self.copy_attn or pass1:
			attn = dec_attn["std"]
			...
			attn = dec_attn["copy"]
			scores = model.generator(dec_out.view(-1, dec_out.size(2)),
	...
	def _score_target(self, batch, memory_bank, src_lengths, data, src_map, pass1=False):
		tgt_in = inputters.make_features(batch, 'tgt')[:-1]

		log_probs, attn = self._decode_and_generate(
			tgt_in, memory_bank, batch, data,
			memory_lengths=src_lengths, src_map=src_map, pass1=pass1)
