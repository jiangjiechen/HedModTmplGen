...
import monmt
from monmt.modules.position_ffn import PositionwiseFeedForward
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout,
        ...
        if self_attn_type == "scaled-dot":
            self.self_attn = monmt.modules.MultiHeadedAttention(
                heads, d_model, dropout=dropout)
        elif self_attn_type == "average":
            self.self_attn = monmt.modules.AverageAttention(
                d_model, dropout=dropout)

        self.context_attn = monmt.modules.MultiHeadedAttention(
            heads, d_model, dropout=dropout)
...
class TransformerDecoder2(nn.Module):
    ...
    Args:
       ...
       embeddings (:obj:`monmt.modules.Embeddings`):
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None, step=None):
        """
        See :obj:`monmt.modules.RNNDecoderBase.forward()`
        """
        if step == 0:
